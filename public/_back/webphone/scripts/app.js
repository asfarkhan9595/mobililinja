...
$(document).ready(function() {
    ...
    ctxSip = {
        ...
        newSession : function(newSess) {
            ...
            if (newSess.direction === 'incoming') {
                status = "Saapuva puhelu: "+ newSess.displayName;
            ...
            newSess.on('progress',function(e) {
                if (e.direction === 'outgoing') {
                    ctxSip.setCallSessionStatus('Soitetaan...');
                }
            });

            newSess.on('connecting',function(e) {
                if (e.direction === 'outgoing') {
                    ctxSip.setCallSessionStatus('Yhdistetään...');
                }
            });
            ...
            newSess.on('accepted',function(e) {
                ...
                ctxSip.stopRingTone();
                ctxSip.setCallSessionStatus('Vastattu');
            ...
            newSess.on('failed',function(e) {
                ctxSip.stopRingTone();
                ctxSip.stopRingbackTone();
                ctxSip.setCallSessionStatus('Lopetettu');
            });

            newSess.on('rejected',function(e) {
                ctxSip.stopRingTone();
                ctxSip.stopRingbackTone();
                ctxSip.setCallSessionStatus('Hylätty');
                ctxSip.callActiveID = null;
                ctxSip.logCall(this, 'ended');
                newSess             = null;
            });
        ...
        getUserMediaFailure : function(e) {
            ...
            ctxSip.setError(true, 'Media virhe.', 'Sinun on sallittava mikrofonin käyttö. Tarkista osoitepalkki.', true);
        },
        ...
        logItem : function(item) {
            ...
            switch (item.status) {
                case 'ringing'  :
                    callClass = 'list-group-item-incoming';
                    callIcon  = 'fa-phone';
                    break;

                case 'missed'   :
                    callClass = 'list-group-item-missed';
                    if (item.flow === "incoming") { callIcon = 'fa-level-down'; }
                    if (item.flow === "outgoing") { callIcon = 'fa-level-up'; }
                    break;
            ...
            i  = '<div class="list-group-item sip-logitem clearfix '+callClass+'" data-uri="'+item.uri+'" data-sessionid="'+item.id+'" title="Soita kaksoisnapsauttamalla">';
            i += '<div class="clearfix"><div class="pull-left">';
            i += '<i class="fa fa-fw '+callIcon+' fa-fw"></i> <strong>'+ctxSip.formatPhone(item.uri)+'</strong><br><small>'+moment(item.start).format('DD.MM.YYYY HH:MM')+'</small>';
            i += '</div>';
            ...
            if (callActive) {
                ...
                if (item.status === 'ringing' && item.flow === 'incoming') {
                    i += '<button class="btn btn-xs btn-success btnCall" title="Soita"><i class="fa fa-phone"></i></button>';
                } else {
                    i += '<button class="btn btn-xs btn-primary btnHoldResume" title="Pito"><i class="fa fa-pause"></i></button>';
                    i += '<button class="btn btn-xs btn-info btnTransfer" title="Yhdistä"><i class="fa fa-random"></i></button>';
                    i += '<button class="btn btn-xs btn-warning btnMute" title="Mykistä"><i class="fa fa-fw fa-microphone"></i></button>';
                }
                i += '<button class="btn btn-xs btn-danger btnHangUp" title="Lopeta"><i class="fa fa-phone"></i></button>';
                i += '</div>';
            }
            i += '</div>';
        ...
        sipTransfer : function(sessionid) {
            ...
            var s      = ctxSip.Sessions[sessionid],
                target = window.prompt('Syötä kohdenumero', '');

            ctxSip.setCallSessionStatus('<i>Puhelu siirretään...</i>');
            s.refer(target);
        },
        ...
        hasWebRTC : function() {
            ...
            if (navigator.webkitGetUserMedia) {
                ...
                ctxSip.setError(true, 'Tukematon selain.', 'Selaimesi ei tue tämän puhelimen edellyttämiä ominaisuuksia.');
                window.console.error("WebRTC-tukea ei löydy");
                return false;
            }
        }
    };
    ...
    ctxSip.phone.on('connected', function(e) {
        ctxSip.setStatus("Yhdistetty");
    });

    ctxSip.phone.on('disconnected', function(e) {
        ctxSip.setStatus("Yhteys katkaistu");

        // disable phone
        ctxSip.setError(true, 'Websocket yhteys katkaistu.', 'Virhe muodostettaessa Websocket yhteyttä.');
    ...
    ctxSip.phone.on('registered', function(e) {
        ...
        var closeEditorWarning = function() {
            return 'Jos suljet tämän ikkunan, et voi soittaa tai vastaanottaa puheluita selaimellasi.';
        };
    ...
    ctxSip.phone.on('registrationFailed', function(e) {
        ctxSip.setError(true, 'Rekisteröintivirhe.', 'Puhelimesi rekisteröimisessä tapahtui virhe. Tarkista asetuksesi.');
        ctxSip.setStatus("Virhe: Yhteys ongelma");
    });
